...
import com.gxtDemo.client.dto.SaveCustomer;
import com.gxtDemo.client.dto.SaveCustomerDTO;
import com.gxtDemo.client.model.Type;
import com.gxtDemo.client.model.TypeProperties;
import com.gxtDemo.client.model.Settlement;
import com.gxtDemo.client.model.SettlementProperties;
...
public class SaveCustomerView {
    private VerticalLayoutContainer verticalLtWidget;

    final TextField tdCode = new TextField(); // 客户代码
    final TextField tdName = new TextField(); // 客户名称
    final TextField tdMnemonicCode = new TextField(); // 助记码

    TypeProperties typeProperties = GWT.create(TypeProperties.class);
    ListStore<Type> listStoreType = new ListStore<>(typeProperties.key());
    ComboBox<Type> cbbType = new ComboBox<>(listStoreType,typeProperties.nameLabel()); // 客户类型
    ...
    CheckBox chkMark = new CheckBox();//启用标记
    final TextField tdCompany = new TextField(); // 工作单位
    final DateTimeWidget dtBirthday = new DateTimeWidget(); // 出生日期
    final TextField tdPostCode = new TextField(); // 邮编
    final TextField tdBankAccount = new TextField();// 银行账号
    final TextField tdBank = new TextField(); // 银行

    SettlementProperties settlementProperties = GWT.create(SettlementProperties.class);
    ListStore<Settlement> listStoreSettlement = new ListStore<>(settlementProperties.key());
    ComboBox<Settlement> cbbSettlement = new ComboBox<Settlement>(listStoreSettlement, settlementProperties.nameLabel());//结算方式

    final DateTimeWidget dtSettlementTime = new DateTimeWidget(); // 结算日期
    final DateTimeWidget dtMonthlyTime = new DateTimeWidget(); // 月结日期
        ...
        if (verticalLtWidget == null){
            ...
            buttonSave.addSelectHandler(new SelectEvent.SelectHandler() {
                ...
                public void onSelect(SelectEvent event) {
                    SaveCustomer request = validate();
                    if (request != null){
                        GxtDemoService.App.getInstance().saveCustomer(request,new SaveCustomerAsyncCallback());
                        GxtDemo.back();
                        Info.display("提示","新增客户成功");
                    }
                }
            });
            //返回按钮
    ...
    private ContentPanel createDetailFields(){
        ...
        taRemark.setSize("200", "100");

        ContentPanel cpDetialInfo =  new ContentPanel();
        cpDetialInfo.setHeight(350);
        cpDetialInfo.setHeading("详细信息");
        ...
        horizonalLt.add(verticalLtLeft,new HorizontalLayoutContainer.HorizontalLayoutData(-1,1,new Margins(0,0,0,20)));
        horizonalLt.add(verticalLtRight,new HorizontalLayoutContainer.HorizontalLayoutData(-1,1,new Margins(0,0,0,20)));
        cpDetialInfo.add(horizonalLt,new MarginData(20,0,0,20));

        return cpDetialInfo;
    }


    private SaveCustomer validate(){
    ...
    public class DateTimeWidget extends Composite {
        ...
        public DateTimeWidget() {
            ...
            hbokLtDate.add(dateField, new BoxLayoutContainer.BoxLayoutData(new Margins(0, 0, 0, 0)));
    ...
    private class SaveCustomerAsyncCallback implements AsyncCallback<SaveCustomerDTO>{
        @Override
        ...
        @Override
        public void onSuccess(SaveCustomerDTO result) {
            if (result != null){
